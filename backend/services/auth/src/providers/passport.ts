    ...
    new GoogleStrategy(
      {
        ...
        callbackURL: `/backend/${
          Locals.config().appId
        }/authentication/signin/google/callback`,
        scope: ["email"],
      },
...
if (
  Locals.config().microsoftClientId &&
  Locals.config().microsoftClientSecret
) {
  passport.use(
    new MicrosoftStrategy(
      {
        ...
        callbackURL: `/backend/${
          Locals.config().appId
        }/authentication/signin/microsoft/callback`,
        scope: ["user.read"],
      },
...
if (Locals.config().githubClientId && Locals.config().githubClientSecret) {
  console.log("GITHUB URL", Locals.config().githubClientId);

  const callbackURL =
    "http://127.0.0.1:9090/backend/auth/authentication/signin/github/callback";
    ...
    new GoogleStrategy(
      {
        ...
        callbackURL: `/backend/${
          Locals.config().appId
        }/authentication/signup/google/callback`,
        scope: ["email"],
      },
      (req, profile, issuer, done) => {
        return done(null, profile?.emails[0]?.value || null);
    ...
    new MicrosoftStrategy(
      {
        ...
        callbackURL: `/backend/${
          Locals.config().appId
        }/authentication/signup/microsoft/callback`,
        scope: ["user.read"],
      },
      (req, issuer, profile, done) => {
        return done(null, profile?.emails[0]?.value || null);
    ...
    new GitHubStrategy(
      {
        ...
        callbackURL: `/backend/${
          Locals.config().appId
        }/authentication/signup/github/callback`,
        scope: ["user:email"],
      },
      (req, issuer, profile, done) => {
        return done(null, profile?.emails[0]?.value || null);
